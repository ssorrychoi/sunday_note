...
class MemoListArgs {
  ...
  final bool sortingStandard;

  MemoListArgs({this.folderName, this.sortingStandard});
}
...
class MemoListScreen extends StatefulWidget {
  ...
  MemoListScreen({this.folderName, this.sortingStandard});
...
class _MemoListScreenState extends State<MemoListScreen> {
  ...
  void initState() {
    ...
    _model.initSharedPreferences(widget.folderName, widget.sortingStandard);
    AnalyticsService().logSetScreen('MemoListScreen');
    print('SORTING  : ${widget.sortingStandard.toString()}');
    // widget.sortingStandard ? _model.changeListing(Strings.newSorting) : _model.changeListing(Strings.oldSorting);
  }
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: CustomScrollView(slivers: [
                              ...
                              Row(
                                ...
                                children: [
                                  Selector<MemoListModel, String>(
                                      selector: (context, data) => data.sortingMemo,
                                      builder: (context, sortingValue, _) {
                                        return DropdownButton(
                                            value: sortingValue,
                                            icon: Icon(Icons.arrow_drop_down),
                                            items: <String>[Strings.oldSorting, Strings.newSorting].map<DropdownMenuItem<String>>((String value) {
                                            ...
                                            onChanged: (String newValue) {
                                              ...
                                              _model.reverseListing(newValue, widget.folderName);
                                              newValue == Strings.oldSorting
                                                  ? SharedPreference.setNewestListSequence(false)
                                                  : SharedPreference.setNewestListSequence(true);
                              ...
                              (context, index) => Padding(
                                  ...
                                  child: Dismissible(
                                    ...
                                    onDismissed: (direction) {
                                      ...
                                      Scaffold.of(context).showSnackBar(SnackBar(content: Text(Strings.removeMemoMsg)));
                                    },
                                        ...
                                        builder: (context, sorting, _) {
                                          return InkWell(
                                            ...
                                            onTap: () {
                                              ...
                                              Navigator.pushNamed(context, Routes.addMemo,
                                                  arguments: AddMemoArgs(
                                                    dateText: Memo.fromJson(jsonDecode(memoList[index])).date.toString(),
                                                    titleText: Memo.fromJson(jsonDecode(memoList[index])).title.toString(),
                                                    wordsText: Memo.fromJson(jsonDecode(memoList[index])).words.toString(),
                                                    contentsText: Memo.fromJson(jsonDecode(memoList[index])).contents.toString(),
                                                    speaker: Memo.fromJson(jsonDecode(memoList[index])).speaker.toString(),
                                                    folderName: widget.folderName,
                                                  )).then((value) {
                                                if (value != null) {
                                                  // print(value.title);
                                                  _model.updateMemoList(widget.folderName, index, value, sorting);
                                                }
                                              });
                                            },
                                            child: MemoListItem(Memo.fromJson(jsonDecode(memoList[index])), widget.folderName),
                                          );
                                        }),
                                  )),
                              childCount: memoCnt));
          ...
          SliverList(delegate: SliverChildListDelegate([const SizedBox(height: 100)]))
        ]),
